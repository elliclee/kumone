...
- 引入 0.3.13 的歌词振假名、罗马音修正、迷你播放器封面缓存和播放页转场锚点修复。
- 引入 0.3.14 的黑胶播放模式，继续以沉浸模式作为默认值。
- 卡片网格采用上游的紧凑尺寸双列判定，同时保留本地 16 pt 页面边距、12 pt 列间距、流式封面宽度和 iOS 简化卡片外观。
- iOS 26 的 mini-player accessory 使用上游单一 `TabView` 身份修复，资料库、漫游页和系统 Tab 结构保持本地精修版本。
- 更新弹窗移除自定义渐变胶囊主按钮，改用系统 prominent 按钮；手动下载使用系统 bordered 按钮，Sheet 使用系统中等高度与拖拽指示器。
- 上游 macOS 单窗口、桌面歌词和播放器状态修复原样保留，不把 iPhone 布局令牌扩散到 macOS。
