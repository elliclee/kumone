...
- ✨ **AI 猜你喜欢** — 根据红心歌曲发现尚未收藏的新歌，生成推荐理由并自动匹配网易云曲库（服务密钥存于系统钥匙串）
...
核心音乐能力不依赖第三方中转服务器：weapi（AES-CBC 双层 + RSA）与 eapi（AES-ECB + MD5 摘要）加密为原生 Swift 实现，请求直达 `music.163.com` / `interface.music.163.com`。可选的“AI 猜你喜欢”由客户端直连推荐服务，使用用户自己的服务密钥。
